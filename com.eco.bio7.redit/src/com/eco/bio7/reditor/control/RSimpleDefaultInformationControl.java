...
import com.eco.bio7.reditor.actions.OpenTemplatePreferences;
 ...
 * @since 2.0
 */
public class RSimpleDefaultInformationControl extends AbstractInformationControl implements DisposeListener {
	...
	public interface IInformationPresenter {
		 ...
		 *             {@link RSimpleDefaultInformationControl.IInformationPresenterExtension#updatePresentation(Drawable, String, TextPresentation, int, int)}
		 */
	...
	public interface IInformationPresenterExtension {
		 ...
		 * <p>
		 * Replaces
		 * {@link RSimpleDefaultInformationControl.IInformationPresenter#updatePresentation(Display, String, TextPresentation, int, int)}
	 ...
	 * @since 3.4
	 */
	public RSimpleDefaultInformationControl(Shell parent, String statusFieldText, IInformationPresenter presenter) {
	 ...
	 * @since 3.4
	 */
	public RSimpleDefaultInformationControl(Shell parent, ToolBarManager toolBarManager, IInformationPresenter presenter) {
	 ...
	 *            the parent shell
	 */
	public RSimpleDefaultInformationControl(Shell parent) {
	 ...
	 */
	public RSimpleDefaultInformationControl(Shell parent, IInformationPresenter presenter) {
	 ...
	 *             {@link #DefaultInformationControl(Shell, RSimpleDefaultInformationControl.IInformationPresenter)}
	 */
	public RSimpleDefaultInformationControl(Shell parent, int textStyles, IInformationPresenter presenter) {
	 ...
	 * @since 3.0
	 * @deprecated As of 3.4, replaced by
	 *             {@link #DefaultInformationControl(Shell, String, RSimpleDefaultInformationControl.IInformationPresenter)}
	 */
	public RSimpleDefaultInformationControl(Shell parent, int textStyles, IInformationPresenter presenter, String statusFieldText) {
		super(parent, statusFieldText);
	...
	public void setInformation(String content) {
		...
		if (content.contains("::::args::::")) {
			content = content.split("::::args::::")[0];
			isArg = true;
		}
		REditorTextHover.setHtmlHelpText(content);
		informationControlText = null;
		if (fPresenter == null) {

			final String finalContent = content;

			getRApiInformation(finalContent);
			...
			if (informationControlText != null) {
				if (informationControlText.isEmpty() == false) {
					...
					if (isArg) {
						...
						int index = informationControlText.indexOf("Arguments:");
						if (index >= 0 && index < fText.getCharCount()) {
							int lineOffset = fText.getLineAtOffset(index);
							/* scroll to the arguments section! */
							fText.setTopIndex(lineOffset);
						}
					}

				} else {
					fText.setText(finalContent);
				}
			} else {
				fText.setText(finalContent);
			}
	...
	private void getRApiInformation(final String finalContent) {
			...
			if (RState.isBusy() == false) {
				...
				display.syncExec(new Runnable() {
					public void run() {
						...
						informationControlText = rObjectInfo.getRHelpInfo(finalContent, c,RStrObjectInformation.PACKAGE_LABEL);
		...
		if (visible) {
			if (fText.getWordWrap()) {
				...
				if ((locLastLine.y + 40) <= 400) {
					setSize(locEndLine.x + 40, locLastLine.y + 80);
				} else {
	...
	public IInformationControlCreator getInformationPresenterControlCreator() {
		return new IInformationControlCreator() {
			...
			public IInformationControl createInformationControl(Shell parent) {
				...
				OpenTemplatePreferences openTemplatePreferences = new OpenTemplatePreferences();
				...
				tbm.add(webHelp);
				tbm.add(openTemplatePreferences);
				tbm.update(true);
