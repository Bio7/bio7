...
public class RDefaultHoverTextmarkerInformationControl extends AbstractInformationControl implements DisposeListener {
	 ...
	 * @since 3.4
	 */
	public RDefaultHoverTextmarkerInformationControl(Shell parent, String statusFieldText,
	 ...
	 * @since 3.4
	 */
	public RDefaultHoverTextmarkerInformationControl(Shell parent, ToolBarManager toolBarManager,
			IInformationPresenter presenter) {
	 ...
	 * @param proposals
	 */
	public RDefaultHoverTextmarkerInformationControl(Shell parent, ICompletionProposal[] proposals, String message,
			REditor rEditor) {
	 ...
	 * @param proposals
	 */
	public RDefaultHoverTextmarkerInformationControl(Shell parent, ToolBarManager tbm,
			IInformationPresenter fPresenter2, ICompletionProposal[] proposals, String message, REditor rEditor) {
	...
	public void setInformation(String content) {
		...
		if (proposals != null) {
							...
							display.asyncExec(new Runnable() {
								...
								public void run() {
									...
									if (parent.isDisposed() == false) {
										if (parent.getShell().isDisposed() == false) {
			...
			if (Util.getOS().equals("Mac")) {
				/*
				 * Set table item height via reflection for MacOSX. Source from:
				 * https://stackoverflow.com/questions/9465732/swt-custom-row-height-on-empty-
				 * tables-trees
				 */
				try {
					/*
					 * Locate the method setItemHeight(int). Note that if you do not have access to
					 * the method, you must use getDeclaredMethod(). If setItemHeight(int) were
					 * public, you could simply call getDeclaredMethod.
					 */
					Method setItemHeightMethod = table.getClass().getDeclaredMethod("setItemHeight", int.class);

					/*
					 * Set the method as accessible. Again, this would not be necessary if
					 * setItemHeight(int) were public.
					 */
					 ...
					 */
					setItemHeightMethod.invoke(table, font.getFontData()[0].getHeight() + 10);
				} catch (Exception e) {
					/*
					 * Reflection failed, it's probably best to swallow the exception and degrade
					 * gracefully, as if we never called setItemHeight. Maybe log the error or print
			...
			// Font font =
			// JFaceResources.getFontRegistry().get("com.eco.bio7.reditor.reditor.textfont");
			...
			/* Add a bit space for the toolbar below! */
			fText.setText(content + "\n");
			/*
			 * TableItem item = new TableItem(table, 0); item.setFont(font);
	 ...
	 * getShell().pack(true); Point newSize = getShell().getSize(); if (newSize.x >
	 * currentSize.x || newSize.y > currentSize.y) setSize(currentSize.x,
	 * currentSize.y); // restore previous // size } }
	 * 
	...
	public IInformationControlCreator getInformationPresenterControlCreator() {
		return new IInformationControlCreator() {
			...
			public IInformationControl createInformationControl(Shell parent) {
				 ...
				 * tbm.add(localBrowserHelp); tbm.add(webHelp); tbm.update(true);
				 */
				return new RDefaultHoverTextmarkerInformationControl(parent, (ToolBarManager) tbm, fPresenter,
						proposals, message, rEditor);
			}
		};
	}
