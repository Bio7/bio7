...
public class VideoLayer implements Renderable {
	 ...
	 * Renders the object.
	 * 
	 * @param dc draw context reference.
	 */
	@Override
	...
	public void compute(DrawContext dc, GL gl) {
		 ...
		 * Position(Angle.fromDegrees(51.0), Angle.fromDegrees(9.0), 1200)); LatLon p2ll
		 * = LatLon.greatCircleEndPosition(pos, marker.getHeading(),
		 * Angle.fromDegrees(.1)); Vec4 p2 =
		 * dc.getGlobe().computePointFromPosition(p2ll.getLatitude(),
		 * p2ll.getLongitude(), pos.getElevation());
		 * 
		 * // This method then performs the same operation as Vec4.axisAngle() but with
		 * a "v2" of <0, 0, 1>. Vec4 p1p2 = p2.subtract3(point).normalize3(); // Compute
		 * rotation angle Angle directionAngle = Angle.fromRadians(Math.acos(p1p2.z));
		 * // Compute the direction cosine factors that define the rotation axis double
		 * A = -p1p2.y; double B = p1p2.x; double L = Math.sqrt(A A + B B);
		 * 
		 * // Compute rotation angle on z (roll) to keep the arrow plane parallel to the
		 * ground Vec4 horizontalVector =
		 * dc.getGlobe().computeSurfaceNormalAtPoint(point).cross3(p1p2); Vec4 rotatedX
		 * = Vec4.UNIT_X.transformBy3(Matrix.fromAxisAngle(directionAngle, A / L, B / L,
		 * 0)); Angle rollAngle = rotatedX.angleBetween3(horizontalVector); // Find out
		 * which way to do the roll double rollDirection =
		 * Math.signum(-horizontalVector.cross3(rotatedX).dot3(p1p2));
		 * 
		 * gl.glRotated(directionAngle.degrees, A / L, B / L, 0); // point arrow toward
		 * p2 gl.glRotated(rollAngle.degrees, 0, 0, rollDirection); // roll arrow to
		 * keep it parallel to the ground double scale = attrs.getHeadingScale();
		 * gl.glScaled(scale, scale, scale); // scale on top of parent sphere radius
	...
	/**
	 * Converts position in cartesian coordinates (XYZ) to spherical (radius, lat,
	 * lon) coordinates.
	 * 
	 * @param x X coordinate
	 * @param y Y coordinate
	 * @param z Z coordinate
	 * @return a <code>Vec4</code> point for the spherical coordinates {radius, lat,
	 *         lon}
	 */
