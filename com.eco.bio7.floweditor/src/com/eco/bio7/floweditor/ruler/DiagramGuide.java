 ...
 * 
 */
public class DiagramGuide implements IGuide {

	/**
	 * Property used to notify listeners when the parts attached to a guide are
	 * changed
	 */
	public static final String PROPERTY_CHILDREN = "guide children changed"; //$NON-NLS-1$
	 ...
	 */
	public void attachPart(IArea part, int alignment) {
		if (getMap().containsKey(part) && getAlignment(part) == alignment) {
			return;
		}

		getMap().put(part, new Integer(alignment));
		IGuide parent = isHorizontal() ? part.getHorizontalGuide() : part
				.getVerticalGuide();
	 ...
	 */
	public void detachPart(IArea part) {
	...
	public int getAlignment(IArea part) {
