...
import com.eco.bio7.reditor.preferences.template.BeanshellContextType;
...
public class TemplateEditorUI  {
	...
	public TemplateStore getTemplateStore() {
		if (fStore == null) {
			fStore= new ContributionTemplateStore(getContextTypeRegistry(), BeanshellEditorPlugin.getDefault().getPreferenceStore(), CUSTOM_TEMPLATES_KEY);
			try {
				fStore.load();
			} catch (IOException e) {
				BeanshellEditorPlugin.getDefault().getLog().log(new Status(IStatus.ERROR, "org.eclipse.ui.examples.javaeditor", IStatus.OK, "", e)); //$NON-NLS-1$ //$NON-NLS-2$
			}
		}
		return fStore;
	}
	...
	public ImageRegistry getImageRegistry() {
		return BeanshellEditorPlugin.getDefault().getImageRegistry();
	}
	...
	public IPreferenceStore getPreferenceStore() {
		return BeanshellEditorPlugin.getDefault().getPreferenceStore();
	}

	public void savePluginPreferences() {
		BeanshellEditorPlugin.getDefault().savePluginPreferences();
	}
