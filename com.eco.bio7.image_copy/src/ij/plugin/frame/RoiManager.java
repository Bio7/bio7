...
public class RoiManager extends PlugInFrame implements ActionListener, ItemListener, MouseListener, MouseWheelListener, ListSelectionListener {
	 ...
	 * String label = (String) listModel.getElementAt(indexes[i]); Roi roi =
	 * (Roi)rois.get(label); IJ.log(i+" "+roi.getName()); } }
	 */
	 ...
	 * "Measure", "Draw", "Show All", "Show None", "Fill", "Deselect",
	 * "Select All", "Combine", "AND", "XOR", "Split", "Sort" or "Multi Measure"
	...
	/*
	 * void selectAll() { boolean allSelected = true; int count = getCount();
	 * for (int i=0; i<count; i++) { if (!list.isIndexSelected(i)) allSelected =
	 * false; } if (allSelected) select(-1); else { for (int i=0; i<count; i++)
