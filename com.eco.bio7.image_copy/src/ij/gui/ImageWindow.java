...
public class ImageWindow extends JFrame implements FocusListener, WindowListener, WindowStateListener, MouseWheelListener {
	...
	public ImageWindow(ImagePlus imp, ImageCanvas ic) {
		 ...
		 */
		/*
		 * boolean openAsHyperStack = imp.getOpenAsHyperStack(); ij =
		 * IJ.getInstance(); this.imp = imp; if (ic==null) {ic=new
		 * ImageCanvas(imp); newCanvas=true;} this.ic = ic; ImageWindow
		 * previousWindow = imp.getWindow(); setLayout(new ImageLayout(ic));
		 * add(ic); addFocusListener(this); addWindowListener(this);
		 * addWindowStateListener(this); addKeyListener(ij);
		 * setFocusTraversalKeysEnabled(false); if (!(this instanceof
		 * StackWindow)) addMouseWheelListener(this); setResizable(true); if
		 * (!(this instanceof
		 * HistogramWindow&&IJ.isMacro()&&Interpreter.isBatchMode())) {
		 * WindowManager.addWindow(this); imp.setWindow(this); }
		 */
		...
		/*
		 * if (previousWindow!=null) { if (newCanvas) setLocationAndSize(false);
		 ...
		 * catch (Exception e) {} } if (centerOnScreen) { GUI.center(this);
		 * centerOnScreen = false; } else if (nextLocation!=null) {
		 * setLocation(nextLocation); nextLocation = null; } if
	...
	private void setLocationAndSize(boolean updating) {
		...
		// maxWindow = new Rectangle(0, maxWindow.y, maxWindow.width,
		// maxWindow.height);
	 ...
	 */
	/*
	 * public boolean close() { boolean isRunning = running || running2; running
	 * = running2 = false; boolean virtual = imp.getStackSize()>1 &&
	 ...
	 * (d.cancelPressed()) return false; else if (d.yesPressed()) { FileSaver fs
	 * = new FileSaver(imp); if (!fs.save()) return false; } } closed = true; if
	 * (WindowManager.getWindowCount()==0) {xloc = 0; yloc = 0;}
	 * WindowManager.removeWindow(this); //setVisible(false); if (ij!=null &&
	...
	/**
	 * Overrides the setBounds() method in Component so we can find out when the
	 * window is resized.
	 */
