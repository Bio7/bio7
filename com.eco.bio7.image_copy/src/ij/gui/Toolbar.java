...
public class Toolbar extends JPanel implements MouseListener, MouseMotionListener, ItemListener, ActionListener {
	...
	private static final String[] builtInTools = { "Arrow", "Brush", "Developer Menu", "Flood Filler", "LUT Menu", "Overlay Brush", "Pencil", "Pixel Inspector", "Spray Can", "Stacks Menu" };
	...
	private void drawButton(Graphics g, int tool) {
		...
		/* Changed for Bio7! */
		fill3DRect(g, index * SIZE, 0, SIZE, SIZE - 1, !down[tool]);
	...
	/**
	 * Returns the size of the brush tool, or 0 if the brush tool is not
	 * enabled.
	 */
	...
	boolean isMacroTool(int tool) {
		return tool >= SPARE1 && tool <= SPARE9 && names[tool] != null && (tools[tool] instanceof MacroToolRunner || names[tool].equals("Unused Tool"));
	}
	 ...
	 * tool ID, or -1 if all tools are in use.
	 */
