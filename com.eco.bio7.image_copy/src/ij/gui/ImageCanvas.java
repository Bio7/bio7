...
public class ImageCanvas extends JPanel implements MouseListener, MouseMotionListener, MouseWheelListener, Cloneable {
	...
	public void paintComponent(Graphics g) {
		...
		try {
			...
			if (img != null)
				g.drawImage(img, 0, 0, (int) (srcRect.width * magnification), (int) (srcRect.height * magnification), srcRect.x, srcRect.y, srcRect.x + srcRect.width, srcRect.y + srcRect.height, null);
			if (overlay != null)
	...
	private void drawOverlay(Overlay overlay, Graphics g) {
		...
		int n = overlay.size();
		if (IJ.debugMode)
			IJ.log("paint: drawing " + n + " overlay ROIs");
	...
	/*
	 * public Graphics getGraphics() { Graphics g = super.getGraphics(); IJ.write("getGraphics: "+count++); if (IJ.altKeyDown()) throw new IllegalArgumentException(""); return g; }
	 */
	...
	private static final double[] zoomLevels = { 1 / 72.0, 1 / 48.0, 1 / 32.0, 1 / 24.0, 1 / 16.0, 1 / 12.0, 1 / 8.0, 1 / 6.0, 1 / 4.0, 1 / 3.0, 1 / 2.0, 0.75, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0, 8.0,
			12.0, 16.0, 24.0, 32.0 };
	...
	/*
	 * public void autoScroll(MouseEvent e) { Roi roi = imp.getRoi(); if (roi==null || roi.getState()!=roi.CONSTRUCTING || srcRect.width>=imageWidth || srcRect.height>=imageHeight || !(roi.getType()==Roi.POLYGON || roi.getType()==Roi.POLYLINE || roi.getType()==Roi.ANGLE)) return; int sx = e.getX();
	 * int sy = e.getY(); xMouseStart = srcRect.x+srcRect.width/2; yMouseStart = srcRect.y+srcRect.height/2; Rectangle r = roi.getBounds(); Dimension size = getSize(); int deltax=0, deltay=0; if (sx<0) deltax = srcRect.width/4; else if (sx>size.width) deltax = -srcRect.width/4; if (sy<0) deltay =
	 * srcRect.height/4; else if (sy>size.height) deltay = -srcRect.height/4; //IJ.log("autoscroll: "+sx+" "+sy+" "+deltax+" "+deltay+" "+r); scroll(screenX(xMouseStart+deltax), screenY(yMouseStart+deltay)); }
	 */
	...
	public void mouseReleased(MouseEvent e) {
		...
		if ((overlay != null || showAllOverlay != null) && ox == mousePressedX && oy == mousePressedY && (System.currentTimeMillis() - mousePressedTime) > 250L && !drawingTool()) {
			if (activateOverlayRoi(ox, oy))
				return;
		}
