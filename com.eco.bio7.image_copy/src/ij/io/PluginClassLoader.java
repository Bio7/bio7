...
/**
 * ImageJ uses this class loader to load plugins and resources from the plugins
 * directory and immediate subdirectories. This class loader will also load
 * classes and resources from JAR files.
 * 
 * <p>
...
public class PluginClassLoader extends URLClassLoader {
	...
	public PluginClassLoader(String path) {
		/* Changed for Bio7! */
		super(new URL[0], Activator.class.getClassLoader());
		init(path);
	}
	...
	private void addDirectory(File f) {
		if (IJ.debugMode)
			IJ.log("PluginClassLoader.addDirectory: " + f);
		try {
	...
	private void addJar(File f) {
		if (f.getName().endsWith(".jar") || f.getName().endsWith(".zip")) {
			if (IJ.debugMode)
				IJ.log("PluginClassLoader.addJar: " + f);
			try {
				addURL(f.toURI().toURL());
			} catch (MalformedURLException e) {
				ij.IJ.log("PluginClassLoader: " + e);
