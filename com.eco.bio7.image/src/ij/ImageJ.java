 ...
 */
public class ImageJ extends JFrame implements ActionListener, MouseListener, KeyListener, WindowListener, ItemListener, Runnable {
	...
	public ImageJ(java.applet.Applet applet, int mode) {
		...
		statusBar.setBackground(this.getSystemColour());
		...
		if (mode != NO_SHOW) {
			...
			setBounds(loc.x, loc.y, ijWidth, ijHeight); // needed for pack to
														// work
			setLocation(loc.x, loc.y);
			pack();
			setResizable(!(IJ.isMacintosh() || IJ.isWindows())); // make
																	// resizable
																	// on Linux
			/* Changed for Bio7! */
			// show();
