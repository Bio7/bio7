...
public class Menus {
	...
	public static final int WINDOW_MENU_ITEMS = 5; // fixed items at top of
													// Window menu
	...
	private static Menu pluginsMenu, saveAsMenu, shortcutsMenu, utilitiesMenu,
			macrosMenu;
	...
	private static int fontSize = Prefs
			.getInt(Prefs.MENU_SIZE, defaultFontSize);
	...
	String addMenuBar() {
		...
		addPlugInItem(file, "Open...", "ij.plugin.Commands(\"open\")",
				KeyEvent.VK_O, false);
		addPlugInItem(file, "Open Next", "ij.plugin.NextImageOpener",
				KeyEvent.VK_O, true);
		getMenu("File>Open Samples", true);
		addOpenRecentSubMenu(file);
		Menu importMenu = getMenu("File>Import", true);
		file.addSeparator();
		addPlugInItem(file, "Close", "ij.plugin.Commands(\"close\")",
				KeyEvent.VK_W, false);
		addPlugInItem(file, "Close All", "ij.plugin.Commands(\"close-all\")",
				0, false);
		addPlugInItem(file, "Save", "ij.plugin.Commands(\"save\")",
				KeyEvent.VK_S, false);
		saveAsMenu = getMenu("File>Save As", true);
		addPlugInItem(file, "Revert", "ij.plugin.Commands(\"revert\")",
				KeyEvent.VK_R, false);
		file.addSeparator();
		addPlugInItem(file, "Page Setup...",
				"ij.plugin.filter.Printer(\"setup\")", 0, false);
		addPlugInItem(file, "Print...", "ij.plugin.filter.Printer(\"print\")",
				KeyEvent.VK_P, false);

		Menu edit = getMenu("Edit");
		addPlugInItem(edit, "Undo", "ij.plugin.Commands(\"undo\")",
				KeyEvent.VK_Z, false);
		edit.addSeparator();
		addPlugInItem(edit, "Cut", "ij.plugin.Clipboard(\"cut\")",
				KeyEvent.VK_X, false);
		addPlugInItem(edit, "Copy", "ij.plugin.Clipboard(\"copy\")",
				KeyEvent.VK_C, false);
		addPlugInItem(edit, "Copy to System", "ij.plugin.Clipboard(\"scopy\")",
				0, false);
		addPlugInItem(edit, "Paste", "ij.plugin.Clipboard(\"paste\")",
				KeyEvent.VK_V, false);
		addPlugInItem(edit, "Paste Control...",
				"ij.plugin.frame.PasteController", 0, false);
		edit.addSeparator();
		addPlugInItem(edit, "Clear", "ij.plugin.filter.Filler(\"clear\")", 0,
				false);
		addPlugInItem(edit, "Clear Outside",
				"ij.plugin.filter.Filler(\"outside\")", 0, false);
		addPlugInItem(edit, "Fill", "ij.plugin.filter.Filler(\"fill\")",
				KeyEvent.VK_F, false);
		addPlugInItem(edit, "Draw", "ij.plugin.filter.Filler(\"draw\")",
				KeyEvent.VK_D, false);
		addPlugInItem(edit, "Invert", "ij.plugin.filter.Filters(\"invert\")",
				KeyEvent.VK_I, true);
		edit.addSeparator();
		...
		gray8Item = addCheckboxItem(imageType, "8-bit",
				"ij.plugin.Converter(\"8-bit\")");
		gray16Item = addCheckboxItem(imageType, "16-bit",
				"ij.plugin.Converter(\"16-bit\")");
		gray32Item = addCheckboxItem(imageType, "32-bit",
				"ij.plugin.Converter(\"32-bit\")");
		color256Item = addCheckboxItem(imageType, "8-bit Color",
				"ij.plugin.Converter(\"8-bit Color\")");
		colorRGBItem = addCheckboxItem(imageType, "RGB Color",
				"ij.plugin.Converter(\"RGB Color\")");
		imageType.add(new MenuItem("-"));
		RGBStackItem = addCheckboxItem(imageType, "RGB Stack",
				"ij.plugin.Converter(\"RGB Stack\")");
		HSBStackItem = addCheckboxItem(imageType, "HSB Stack",
				"ij.plugin.Converter(\"HSB Stack\")");
		image.add(imageType);

		image.addSeparator();
		getMenu("Image>Adjust", true);
		addPlugInItem(image, "Show Info...", "ij.plugin.filter.Info",
				KeyEvent.VK_I, false);
		addPlugInItem(image, "Properties...",
				"ij.plugin.filter.ImageProperties", KeyEvent.VK_P, true);
		...
		image.addSeparator();
		addPlugInItem(image, "Crop", "ij.plugin.Resizer(\"crop\")",
				KeyEvent.VK_X, true);
		addPlugInItem(image, "Duplicate...", "ij.plugin.Duplicator",
				KeyEvent.VK_D, true);
		addPlugInItem(image, "Rename...",
				"ij.plugin.SimpleCommands(\"rename\")", 0, false);
		addPlugInItem(image, "Scale...", "ij.plugin.Scaler", KeyEvent.VK_E,
				false);
		...
		addPlugInItem(process, "Smooth",
				"ij.plugin.filter.Filters(\"smooth\")", KeyEvent.VK_S, true);
		addPlugInItem(process, "Sharpen",
				"ij.plugin.filter.Filters(\"sharpen\")", 0, false);
		addPlugInItem(process, "Find Edges",
				"ij.plugin.filter.Filters(\"edge\")", 0, false);
		addPlugInItem(process, "Find Maxima...",
				"ij.plugin.filter.MaximumFinder", 0, false);
		addPlugInItem(process, "Enhance Contrast...",
				"ij.plugin.ContrastEnhancer", 0, false);
		...
		addPlugInItem(process, "Image Calculator...",
				"ij.plugin.ImageCalculator", 0, false);
		addPlugInItem(process, "Subtract Background...",
				"ij.plugin.filter.BackgroundSubtracter", 0, false);
		addItem(process, "Repeat Command", KeyEvent.VK_R, true);

		Menu analyzeMenu = getMenu("Analyze");
		addPlugInItem(analyzeMenu, "Measure", "ij.plugin.filter.Analyzer",
				KeyEvent.VK_M, false);
		addPlugInItem(analyzeMenu, "Analyze Particles...",
				"ij.plugin.filter.ParticleAnalyzer", 0, false);
		addPlugInItem(analyzeMenu, "Summarize",
				"ij.plugin.filter.Analyzer(\"sum\")", 0, false);
		addPlugInItem(analyzeMenu, "Distribution...", "ij.plugin.Distribution",
				0, false);
		addPlugInItem(analyzeMenu, "Label",
				"ij.plugin.filter.Filler(\"label\")", 0, false);
		addPlugInItem(analyzeMenu, "Clear Results",
				"ij.plugin.filter.Analyzer(\"clear\")", 0, false);
		addPlugInItem(analyzeMenu, "Set Measurements...",
				"ij.plugin.filter.Analyzer(\"set\")", 0, false);
		analyzeMenu.addSeparator();
		addPlugInItem(analyzeMenu, "Set Scale...",
				"ij.plugin.filter.ScaleDialog", 0, false);
		addPlugInItem(analyzeMenu, "Calibrate...",
				"ij.plugin.filter.Calibrator", 0, false);
		addPlugInItem(analyzeMenu, "Histogram", "ij.plugin.Histogram",
				KeyEvent.VK_H, false);
		addPlugInItem(analyzeMenu, "Plot Profile",
				"ij.plugin.filter.Profiler(\"plot\")", KeyEvent.VK_K, false);
		addPlugInItem(analyzeMenu, "Surface Plot...",
				"ij.plugin.SurfacePlotter", 0, false);
		...
		addPlugInItem(window, "Show All",
				"ij.plugin.WindowOrganizer(\"show\")",
				KeyEvent.VK_CLOSE_BRACKET, false);
		addPlugInItem(window, "Put Behind [tab]",
				"ij.plugin.Commands(\"tab\")", 0, false);
		addPlugInItem(window, "Cascade",
				"ij.plugin.WindowOrganizer(\"cascade\")", 0, false);
		addPlugInItem(window, "Tile", "ij.plugin.WindowOrganizer(\"tile\")", 0,
				false);
		window.addSeparator();

		Menu help = getMenu("Help");
		addPlugInItem(help, "ImageJ Website...", "ij.plugin.BrowserLauncher",
				0, false);
		addPlugInItem(help, "ImageJ News...", "ij.plugin.BrowserLauncher(\""
				+ IJ.URL + "/notes.html\")", 0, false);
		addPlugInItem(help, "Documentation...", "ij.plugin.BrowserLauncher(\""
				+ IJ.URL + "/docs\")", 0, false);
		addPlugInItem(help, "Installation...",
				"ij.plugin.SimpleCommands(\"install\")", 0, false);
		addPlugInItem(
				help,
				"Mailing List...",
				"ij.plugin.BrowserLauncher(\"https://list.nih.gov/archives/imagej.html\")",
				0, false);
		help.addSeparator();
		addPlugInItem(help, "Dev. Resources...", "ij.plugin.BrowserLauncher(\""
				+ IJ.URL + "/developer/index.html\")", 0, false);
		addPlugInItem(help, "Plugins...", "ij.plugin.BrowserLauncher(\""
				+ IJ.URL + "/plugins\")", 0, false);
		addPlugInItem(help, "Macros...", "ij.plugin.BrowserLauncher(\""
				+ IJ.URL + "/macros/\")", 0, false);
		addPlugInItem(help, "Macro Functions...",
				"ij.plugin.BrowserLauncher(\"" + IJ.URL
						+ "/developer/macro/functions.html\")", 0, false);
		help.addSeparator();
		/* Changed for Bio7! Left out ImageJ update! */
		// addPlugInItem(help, "Update ImageJ...", "ij.plugin.ImageJ_Updater",
		// 0, false);
		addPlugInItem(help, "Refresh Menus",
				"ij.plugin.ImageJ_Updater(\"menus\")", 0, false);
		help.addSeparator();
	...
	void addPlugInItem(Menu menu, String label, String className, int shortcut,
			boolean shift) {
	...
	static void addPluginItem(Menu submenu, String s) {
		...
		if (command.endsWith("]")) {
			...
			if (openBracket > 0) {
				String shortcut = command.substring(openBracket + 1,
						command.length() - 1);
				keyCode = convertShortcutToCode(shortcut);
				boolean functionKey = keyCode >= KeyEvent.VK_F1
	...
	void installJarPlugin(String jar, String s) {
		...
		if (s.startsWith("Plugins>")) {
			...
			String name = firstQuote < 0 ? s : s.substring(0, firstQuote)
					.trim();
			...
			if (offset < 0) {
				...
				if (readFromProps)
					...
					addItemSorted(parentMenu, result,
							parentName.equals("Plugins") ? userPluginsIndex : 0);
				else
	...
	InputStream autoGenerateConfigFile(String jar) {
		...
		try {
			...
			while (entries.hasMoreElements()) {
				...
				if (name.endsWith(".class") && name.indexOf("_") > 0
					...
					if (slashIndex >= 0) {
						plugins += ">"
								+ className.substring(0, slashIndex)
										.replace('/', '>').replace('_', ' ');
	...
	String[] getStrippedPlugins(String[] plugins) {
		...
		for (int i = 0; i < plugins2.length; i++) {
			...
			if (slashPos >= 0)
				plugins2[i] = plugins[i].substring(slashPos + 1,
						plugins2[i].length());
		}
		return plugins2;
	}
	...
	void setupPluginsAndMacrosPaths() {
		/*
		 * pluginsPath = macrosPath = null; String homeDir = Prefs.getHomeDir();
		 * if (homeDir==null) return; if (homeDir.endsWith("plugins"))
		 * pluginsPath = homeDir+Prefs.separator; else { String property =
		 * System.getProperty("plugins.dir"); if (property!=null &&
		 * (property.endsWith("/")||property.endsWith("\\"))) property =
		 * property.substring(0, property.length()-1); String pluginsDir =
		 * property; if (pluginsDir==null) pluginsDir = homeDir; else if
		 * (pluginsDir.equals("user.home")) { pluginsDir =
		 * System.getProperty("user.home"); if (!(new
		 * File(pluginsDir+Prefs.separator+"plugins")).isDirectory()) pluginsDir
		 * = pluginsDir + Prefs.separator + "ImageJ"; property = null; // needed
		 * to run plugins when ImageJ launched using Java WebStart if
		 * (applet==null) System.setSecurityManager(null); jnlp = true; }
		 * pluginsPath = pluginsDir+Prefs.separator+"plugins"+Prefs.separator;
		 * if (property!=null&&!(new File(pluginsPath)).isDirectory())
		 * pluginsPath = pluginsDir + Prefs.separator; macrosPath =
		 * pluginsDir+Prefs.separator+"macros"+Prefs.separator; } File f =
		 * macrosPath!=null?new File(macrosPath):null; if (f!=null &&
		 * !f.isDirectory()) macrosPath = null; f = pluginsPath!=null?new
		 * File(pluginsPath):null; if (f==null || (f!=null && !f.isDirectory()))
		 * { pluginsPath = null; return; }
		 */
		...
		if (f != null && !f.isDirectory()) {
			macrosPath = currentDir + File.separator + "macros"
					+ File.separator;
	...
	private static boolean validMacroName(String name, boolean hasUnderscore) {
		return (hasUnderscore && name.endsWith(".txt"))
				|| name.endsWith(".ijm") || name.endsWith(".js")
				|| (name.endsWith(".bsh") && !isFiji)
				|| (name.endsWith(".py") && !isFiji);
	}
	...
	public static void updateMenus() {
		...
		if (type == ImagePlus.GRAY8) {
			...
			if (ip != null
					...
					&& ip.isColorLut() && !ip.isPseudoColorLut()) {
	...
	static synchronized void insertWindowMenuItem(Window win) {
		if (ij == null || win == null)
			return;
		String title = win instanceof Frame ? ((Frame) win).getTitle()
				: ((Dialog) win).getTitle();
	...
	public static synchronized void updateWindowMenuItem(String oldLabel,
			String newLabel) {
	 ...
	 */
	public static int installPlugin(String plugin, char menuCode,
			String command, String shortcut, ImageJ ij) {
		...
		pluginsPrefs.addElement(menuCode + ",\"" + command + shortcut + "\","
				+ plugin);
		return NORMAL_RETURN;
	}
	...
	void installStartupMacroSet() {
		...
		if (macrosPath == null) {
			(new MacroInstaller())
					.installFromIJJar("/macros/StartupMacros.txt");
			return;
		}
	...
	public static Font getFont() {
		if (menuFont == null)
			menuFont = new Font("SanSerif", Font.PLAIN, fontSize == 0 ? 12
					: fontSize);
		return menuFont;
	}
	...
	public static void updateImageJMenus() {
		...
		IJ.showStatus("Menus updated: " + m.nPlugins + " commands, "
				+ m.nMacros + " macros");
	}
}
