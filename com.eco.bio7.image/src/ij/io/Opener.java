...
public class Opener {
	...
	/**
	 * Displays a JFileChooser and then opens the tiff, dicom, fits, pgm, jpeg, bmp, gif, lut, roi, or text files selected by the user. Displays error messages if one or more of the selected files is not in one of the supported formats. This is the method that ImageJ's File/Open command uses to open
	 * files if "Open/Save Using JFileChooser" is checked in EditOptions/Misc.
	 */
	...
	/**
	 * Attempts to open the specified file as a tiff, bmp, dicom, fits, pgm, gif or jpeg. Displays a file open dialog if 'path' is null or an empty string. Returns an ImagePlus object if successful.
	 */
