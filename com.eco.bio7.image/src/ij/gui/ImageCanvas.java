...
public class ImageCanvas extends JPanel implements MouseListener, MouseWheelListener, MouseMotionListener, Cloneable {
	...
	void drawRoiLabel(Graphics g, int index, Roi roi) {
		...
		// if (IJ.debugMode && index==0)
		// IJ.log("drawRoiLabel: "+drawingList+" "+label+" "+x+" "+y);
	 ...
	 * make the source rectangle (srcRect) smaller and center it at (sx,sy).
	...
	/*
	 * public void autoScroll(MouseEvent e) { Roi roi = imp.getRoi(); if
	 ...
	 * roi.getType()==Roi.ANGLE)) return; int sx = e.getX(); int sy = e.getY();
	 * xMouseStart = srcRect.x+srcRect.width/2; yMouseStart =
	 * srcRect.y+srcRect.height/2; Rectangle r = roi.getBounds(); Dimension size
	 * = getSize(); int deltax=0, deltay=0; if (sx<0) deltax = srcRect.width/4;
	 * else if (sx>size.width) deltax = -srcRect.width/4; if (sy<0) deltay =
	 * srcRect.height/4; else if (sy>size.height) deltay = -srcRect.height/4;
