...
public class Analyzer implements PlugInFilter, Measurements {
	...
	private static final int[] list = { AREA, MEAN, STD_DEV, MODE, MIN_MAX,
			CENTROID, CENTER_OF_MASS, PERIMETER, RECT, ELLIPSE,
			SHAPE_DESCRIPTORS, FERET, INTEGRATED_DENSITY, MEDIAN, SKEWNESS,
			KURTOSIS, AREA_FRACTION, STACK_POSITION, LIMIT, LABELS, INVERT_Y,
			SCIENTIFIC_NOTATION, ADD_TO_OVERLAY };
	 ...
	 * current measurement options and default results table.
	 */
	...
	void setOptions(GenericDialog gd) {
		...
		if ((oldMeasurements & (~LIMIT) & (~SCIENTIFIC_NOTATION)) != (systemMeasurements
				& (~LIMIT) & (~SCIENTIFIC_NOTATION))
				&& IJ.isResultsWindow()) {
			rt.setPrecision((systemMeasurements & SCIENTIFIC_NOTATION) != 0 ? -precision
					: precision);
			clearSummary();
	...
	/**
	 * Measures the image or selection and adds the results to the default
	 * results table.
	 */
	public void measure() {
	...
	private ImagePlus getRedirectImageOrStack(ImagePlus cimp) {
		...
		if (rimp != null) {
			...
			if (depth > 1 && depth == cimp.getStackSize()
					&& rimp.getCurrentSlice() != cimp.getCurrentSlice())
	...
	public static ImagePlus getRedirectImage(ImagePlus cimp) {
		...
		if (rimp.getWidth() != cimp.getWidth()
				|| rimp.getHeight() != cimp.getHeight()) {
			IJ.error("Analyzer", "Redirect image (\"" + redirectTitle
	...
	ImageStatistics getRedirectStats(int measurements, Roi roi) {
		...
		if (imp.getTitle().equals("mask") && imp.getBitDepth() == 8) {
			...
			ip.setRoi(roi);
		return ImageStatistics.getStatistics(ip, measurements,
				redirectImp.getCalibration());
	}
	...
	void measurePoint(Roi roi) {
		...
		ImagePlus imp2 = isRedirectImage() ? getRedirectImageOrStack(imp)
				: null;
		if (imp2 == null)
			imp2 = imp;
		for (int i = 0; i < p.npoints; i++) {
			...
			ImageStatistics stats = ImageStatistics.getStatistics(ip,
					measurements, imp2.getCalibration());
	...
	void measureLength(Roi roi) {
		...
		if (straightLine && lineWidth > 1) {
			...
			if (straightLine) {
				Line l = (Line) roi;
				if ((l.y1 == l.y2 || l.x1 == l.x2) && l.x1 == l.x1d
						&& l.y1 == l.y1d && l.x2 == l.x2d && l.y2 == l.y2d)
					ip2.setRoi(0, 0, ip2.getWidth() - 1, 1);
			}
		}
		ImageStatistics stats = ImageStatistics.getStatistics(ip2, AREA + MEAN
				+ STD_DEV + MODE + MIN_MAX, imp2.getCalibration());
		if (saveR != null)
			ip2.setRoi(saveR);
		if ((roi instanceof Line) && (measurements & CENTROID) != 0) {
			FloatPolygon p = ((Line) roi).getFloatPoints();
			stats.xCentroid = p.xpoints[0] + (p.xpoints[1] - p.xpoints[0])
					/ 2.0;
			stats.yCentroid = p.ypoints[0] + (p.ypoints[1] - p.ypoints[0])
					/ 2.0;
			if (imp2 != null) {
	 ...
	 * calling setMeasurements(), in the default results table.
	 */
	public void saveResults(ImageStatistics stats, Roi roi) {
		if (rt.getColumnHeading(ResultsTable.LAST_HEADING) == null)
			reset();
		clearSummary();
		Measure m = new Measure();// Changed for Bio7 - saveResults called
									// multiple times!
		incrementCounter();
		int counter = rt.getCounter();
		if (counter <= MAX_STANDARDS
				&& !(stats.umean == 0.0 && counter == 1 && umeans != null && umeans[0] != 0f)) {
			if (umeans == null)
		...
		if ((measurements & CENTROID) != 0) {
			...
			m.setCENTROIDX(stats.xCentroid);
			m.setCENTROIDY(stats.yCentroid);
		}
		if ((measurements & CENTER_OF_MASS) != 0) {
			...
			m.setCENTER_OF_MASSX(stats.xCenterOfMass);
			m.setCENTER_OF_MASSY(stats.yCenterOfMass);
		}
		if ((measurements & PERIMETER) != 0
			...
			if (roi != null)
				...
				perimeter = imp != null ? imp.getWidth() * 2 + imp.getHeight()
						* 2 : 0.0;
			if ((measurements & PERIMETER) != 0)
				rt.addValue(ResultsTable.PERIMETER, perimeter);
			m.setPERIMETER(perimeter);
			if ((measurements & SHAPE_DESCRIPTORS) != 0) {
				double circularity = perimeter == 0.0 ? 0.0 : 4.0 * Math.PI
						* (stats.area / (perimeter * perimeter));
				if (circularity > 1.0)
					circularity = 1.0;
				rt.addValue(ResultsTable.CIRCULARITY, circularity);
				m.setCIRCULARITY(circularity);
				Polygon ch = null;
				boolean isArea = roi == null || roi.isArea();
				double convexArea = roi != null ? getArea(roi.getConvexHull())
						: stats.pixelCount;
				rt.addValue(ResultsTable.ASPECT_RATIO, isArea ? stats.major
						/ stats.minor : 0.0);
				m.setASPECT_RATIO(isArea ? stats.major / stats.minor : 0.0);
				rt.addValue(ResultsTable.ROUNDNESS, isArea ? 4.0 * stats.area
						/ (Math.PI * stats.major * stats.major) : 0.0);
				m.setROUNDNESS(isArea ? 4.0 * stats.area
						/ (Math.PI * stats.major * stats.major) : 0.0);
				rt.addValue(ResultsTable.SOLIDITY, isArea ? stats.pixelCount
						/ convexArea : Double.NaN);
				m.setSOLIDITY(isArea ? stats.pixelCount / convexArea
						: Double.NaN);
				// rt.addValue(ResultsTable.CONVEXITY, getConvexPerimeter(roi,
				// ch)/perimeter);
			}
		}
		...
		if ((measurements & STACK_POSITION) != 0) {
			...
			if (imp != null && (imp.isHyperStack() || imp.isComposite())) {
				...
				m.setSLICE(imp != null ? imp.getCurrentSlice() : 1.0);
			}
			if (update && rt == systemRT)
				rt.update(measurements, imp, roi);
		}
		if (roi != null) {
			if (roi.isLine()) {
				rt.addValue("Length", roi.getLength());
				m.setLENGTH(roi.getLength());
				if (roi.getType() == Roi.LINE) {
					double angle = 0.0;
					Line l = (Line) roi;
		...
		/* Add the measure */
		Bio7ImageJAnalyse.measurement.add(m);// Changed for Bio7!
	}

	private void clearSummary() {
		if (summarized && rt.getCounter() >= 4
				&& "Max".equals(rt.getLabel(rt.getCounter() - 1))) {
			for (int i = 0; i < 4; i++)
	...
	final double getArea(Polygon p) {
		...
		for (int i = 0; i < p.npoints; i++) {
			...
			if (iminus1 < 0)
				iminus1 = p.npoints - 1;
			carea += (p.xpoints[i] + p.xpoints[iminus1])
					* (p.ypoints[i] - p.ypoints[iminus1]);
		}
		return (Math.abs(carea / 2.0));
	}

	/*
	 * final double getConvexPerimeter(Roi roi, Polygon ch) { if (roi==null ||
	 * ch==null || !(roi instanceof PolygonRoi)) return 0.0; int[] xp =
	 * ((PolygonRoi)roi).getXCoordinates(); int[] yp =
	 * ((PolygonRoi)roi).getYCoordinates(); int n =
	 * ((PolygonRoi)roi).getNCoordinates(); double perim = getPerimeter(xp, yp,
	 * n); double convexPerim = getPerimeter(ch.xpoints, ch.ypoints,
	 * ch.npoints); return convexPerim; }
	 * 
	 * final double getPerimeter(int[] xp, int yp[], int n) { double dx, dy,
	 * perim=0.0; for (int i=0; i<n-1; i++) { dx = xp[i+1]-xp[i]; dy =
	 * yp[i+1]-yp[i]; perim += Math.sqrt(dx*dx+dy*dy); } dx = xp[n-1] - xp[0];
	 * dy = yp[n-1] - yp[0]; perim += Math.sqrt(dx*dx+dy*dy); return perim; }
	 */
	...
	void savePoints(Roi roi) {
		...
		if (imp.getProperty("FHT") != null) {
			...
			y = imp.getHeight() - y - 1;
			double r = Math.sqrt((x - center) * (x - center) + (y - center)
					* (y - center));
			if (r < 1.0)
				r = 1.0;
	...
	String getFileName() {
		...
		if (imp != null) {
			...
			// !Character.isDigit(s.charAt(len-1)))
	...
	public void summarize() {
		...
		for (int col = first; col < columns; col++) {
			...
			rt.setValue(headings[col], n + 1,
					Math.sqrt((sum2[col] - sum[col] * sum[col] / n) / (n - 1)));
	...
	public synchronized static boolean resetCounter() {
		...
		boolean macro = (IJ.macroRunning() && !switchingModes)
				|| Interpreter.isBatchMode();
		switchingModes = false;
		if (counter > 0 && lineCount > 0 && unsavedMeasurements && !macro
				&& ij != null && !ij.quitting()) {
			YesNoCancelDialog d = new YesNoCancelDialog(ij, "ImageJ", "Save "
	...
	/**
	 * Returns the default results table. This table should only be displayed in
	 * a the "Results" window.
	 */
	...
	/**
	 * Returns an updated Y coordinate based on the current
	 * "Invert Y Coordinates" flag.
	 */
