...
public class ColorThresholder extends PlugInFrame implements PlugIn, Measurements, ActionListener, AdjustmentListener, FocusListener, ItemListener, Runnable {
	...
	/**
	 * Converts the current image from RGB to CIE L*a*b* and stores the results in the same RGB image R=L*, G=a*, B=b*. Values are therfore offset and rescaled. see: http://www.brucelindbloom.com/index.html?WorkingSpaceInfo.html#Specifications http://www.easyrgb.com/math.php?MATH=M7#text7
	 * 
	...
	class BandPlot extends Canvas implements Measurements, MouseListener {
		...
		/**
		 * Overrides Component getPreferredSize(). Added to work around a bug in Java 1.4 on Mac OS X.
		 */
