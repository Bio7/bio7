...
public class OBJModel {
	...
	/*
	 * public OBJModel(String nm, GL gl) { this(nm,null, 1.0f, gl, false);
	 *  }
	 */

	public OBJModel(String nm, String path, float sz, GL2 gl, boolean showDetails) {
			...
			while (((line = br.readLine()) != null) && isLoaded) {
				...
				if (line.length() > 0) {
					...
					if (line.startsWith("v ")) { // vertex
						...
						materials = new Materials(line.substring(7), MODEL_DIR,gl);
	...
	/*
	 * The line starts with an OBJ word ("v" or "vn"), followed by three floats
	...
	/*
	 * Add the texture coordinate from the line "vt x y z" to the texCoords
	...
	/*
	 * The line starts with a "vt" OBJ word and two or three floats (x, y, z)
	 * for the tex coords separated by spaces. If there are only two coords,
	 * then the z-value is assigned a dummy value, DUMMY_Z_TC.
	 */
	{
		StringTokenizer tokens = new StringTokenizer(line, " ");
