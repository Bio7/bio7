...
public class ExecuteBatchFile {
	...
	public static void intercompile(String fileprop, String fileextension) {
		...
		/* Compiles a Java file! */
			...
			/* Compile a classbody! */
			/*
			 * if (classbody) { String result = BatchModel.fileToString(fileprop);
			 * 
		...
		/* Starts R with the specified RData file as argument(Windows!) */
			...
			// proc = rt.exec(store.getString(PreferenceConstants.PATH_R) + "\\bin\\Rgui " +
			// result);
		...
		/* Starts the registered pdf application with the file! */
			...
			if (RServe.isAlive()) {
				if (RState.isBusy() == false) {
					...
					if (fileext.equals("html")) {
						 ...
						 * 
						 * public void run() { String temp = "file:///" + dir2 + "/" + theName +
						 * ".html"; String url = temp.replace("\\", "/"); System.out.println(url);
						 * BrowserView b = BrowserView.getBrowserInstance(); b.setLocation(url); } });
						 */
			...
			// pdfLatexPath+"/pdflatex -interaction=nonstopmode " + "-output-directory=" +
			// tempDirLoc + " " + tempDirLoc + "/" + theName + ".tex");
			...
			new Thread() {
				...
				public void run() {
					...
					/*
					 * IWorkspaceRoot root = ResourcesPlugin.getWorkspace().getRoot(); IProject proj
					 * = root.getProject(activeProject.getName()); try {
					 * proj.refreshLocal(IResource.DEPTH_INFINITE, null); } catch (CoreException e)
