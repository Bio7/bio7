...
public class RInterpreterJobStartGui extends WorkspaceJob {
	...
	public void interpret_r_job(String selected) {
		...
		if (ApplicationWorkbenchWindowAdvisor.getOS().equals("Windows")) {
			...
			path = store.getString(PreferenceConstants.PATH_R) + "/bin/x64/Rgui";
			...
			if (shell == 1) {
				...
				// Runtime.getRuntime().exec("xterm -hold -e " + path); //$NON-NLS-1$
				// //$NON-NLS-2$
				List<String> args = new ArrayList<String>();
				args.add("xterm");
				args.add("-hold");
				...
				// Runtime.getRuntime().exec("gnome-terminal -x " + path); //$NON-NLS-1$
				// //$NON-NLS-2$
				List<String> args = new ArrayList<String>();
			...
			// Runtime.getRuntime().exec("/usr/x11/bin/xterm -hold -e " + path);
			// //$NON-NLS-1$ //$NON-NLS-2$
			List<String> args = new ArrayList<String>();
