...
class ClusterJob extends WorkspaceJob {
	...
	public void clusterImage(IProgressMonitor monitor) {
		...
		display.syncExec(new Runnable() {
			...
			public void run() {
				selectionType = ImageMethods.getTransferTypeCombo().getSelectionIndex();
			}
		});
		...
		if (c != null) {
			...
			if (onlyTransferImages == false) {
				...
				try {
					...
					c.eval("try(clarax <- clara(clusterData," + cluster + "," + textOptions + "))");

					c.eval("try(imageCluster<-as.raw(clarax$cluster))");
					c.eval("remove(clusterData)");
					c.eval("remove(clarax);gc();");
				} catch (RserveException e1) {
