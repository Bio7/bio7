	...
	private void createActions() {
		...
		removeStateAction = new Action("(-)") {
			...
			public void run() {
				...
				String stateFromTextfield=txt.getText();
				StateTable.unsetCell(stateFromTextfield);
				Grid grid = StateTable.grid;
				for (int i = 0; i < grid.getItemCount(); i++) {
					GridItem it=grid.getItem(i);
					if (it.getChecked(3) == true||it.getText().equals(stateFromTextfield)) {

						list.add(i);

						String state = it.getText(0);
