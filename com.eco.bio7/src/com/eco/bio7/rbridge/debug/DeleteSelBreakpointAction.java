			...
			if (resource != null) {
				...
				if (selection!=null&&!selection.isEmpty()) {
					...
					for (int i = 0; i < markersfind.length; i++) {
						//System.out.println("line: " + line + "marker: "+ markersfind[i].getAttribute(IMarker.LINE_NUMBER, -1));
						if (line+1 == markersfind[i]
								.getAttribute(IMarker.LINE_NUMBER, -1)) {
							try {
								String func = (String) markersfind[i]
										.getAttribute(IMarker.TEXT);
								System.out.println(func);
								System.out.println("untrace(" + func + ")");
								ConsolePageParticipant.pipeInputToConsole("untrace("
										+ func + ")", true, false);
