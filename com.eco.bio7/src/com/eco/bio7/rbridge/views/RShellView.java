...
public class RShellView extends ViewPart {
	...
	private Menu menu;
	protected String[] listObjectsArray;
				...
				if (RServe.isAliveDialog()) {
					if (RState.isBusy() == false) {
						...
						try {
							...
							con.eval(".bio7TempVarEnvironment$workspVar<-NULL;try(for(i in 1:length(ls())){if(is.matrix(get(ls()[i]))==TRUE){.bio7TempVarEnvironment$workspVar<-append(.bio7TempVarEnvironment$workspVar,ls()[i])}})");
				...
				if (RServe.isAliveDialog()) {
					if (RState.isBusy() == false) {
						...
						try {
							...
							con.eval(".bio7TempVarEnvironment <- new.env()");
							con.eval(".bio7TempVarEnvironment$workspVar<-NULL;try(for(i in 1:length(ls())){if(is.vector(get(ls()[i]))==TRUE){.bio7TempVarEnvironment$workspVar<-append(.bio7TempVarEnvironment$workspVar,ls()[i])}})");
							x = RServe.getConnection().eval(".bio7TempVarEnvironment$workspVar");
							if (x.isNull() == false) {
								try {
									v = x.asStrings();
				...
				if (RServe.isAliveDialog()) {
					if (RState.isBusy() == false) {
						...
						try {
							...
							con.eval(".bio7TempVarEnvironment <- new.env()");
							con.eval(".bio7TempVarEnvironment$workspVar<-NULL;try(for(i in 1:length(ls())){if(is.function(get(ls()[i]))==TRUE){.bio7TempVarEnvironment$workspVar<-append(.bio7TempVarEnvironment$workspVar,ls()[i])}})");
							x = RServe.getConnection().eval(".bio7TempVarEnvironment$workspVar");
							if (x.isNull() == false) {
								try {
									v = x.asStrings();
				...
				if (RServe.isAliveDialog()) {
					if (RState.isBusy() == false) {
						...
						try {
							...
							con.eval(".bio7TempVarEnvironment <- new.env()");
							con.eval(".bio7TempVarEnvironment$workspVar<-NULL;try(for(i in 1:length(ls())){if(is.factor(get(ls()[i]))==TRUE){.bio7TempVarEnvironment$workspVar<-append(.bio7TempVarEnvironment$workspVar,ls()[i])}})");
							x = RServe.getConnection().eval(".bio7TempVarEnvironment$workspVar");
							if (x.isNull() == false) {
								try {
									v = x.asStrings();
				...
				if (RServe.isAliveDialog()) {
					if (RState.isBusy() == false) {
						...
						try {
							...
							con.eval(".bio7TempVarEnvironment <- new.env()");
							con.eval(".bio7TempVarEnvironment$workspVar<-NULL;try(for(i in 1:length(ls())){if(is.data.frame(get(ls()[i]))==TRUE){.bio7TempVarEnvironment$workspVar<-append(.bio7TempVarEnvironment$workspVar,ls()[i])}})");
							x = RServe.getConnection().eval(".bio7TempVarEnvironment$workspVar");
							if (x.isNull() == false) {
								try {
									v = x.asStrings();
	 ...
	 * @throws BadLocationException in case <code>startPosition</code> is invalid in
	 *                              the document
	 */
	...
	private void setHistoryTextInREditor() {
	...
	private void setTxtREditor(String txt) {
		if (txt.length() > 0) {
			IEditorPart editor = (IEditorPart) PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().getActiveEditor();
			if (editor != null && editor instanceof REditor || editor instanceof MarkdownEditor) {
				...
				editor2.selectAndReveal(off + txt.length(), 0);
				// editor2.setFocus();//Focus with cursor blinking!
			}
		}
	}
