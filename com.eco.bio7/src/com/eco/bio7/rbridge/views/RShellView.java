...
public class RShellView extends ViewPart {
	...
	public void createPartControl(final Composite parent) {
		...
		// initializeToolBar(); Error in WindowBuilder!
		...
		text.addMouseListener(new MouseListener() {
			...
			public void mouseDown(MouseEvent e) {
				if (keyPressed) {
					...
					if (textBehingClosingCaret.equals(")") || textBehingClosingCaret.equals("]")
							|| textBehingClosingCaret.equals("}")) {
						Document doc = new Document();
						switch (textBeforeCaret) {
						case ")":
							xch = '(';
		...
		text.addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if (ApplicationWorkbenchWindowAdvisor.getOS().equals("Mac")) {
					...
					if (((e.stateMask & SWT.COMMAND) == SWT.COMMAND) && (e.stateMask & SWT.SHIFT) == SWT.SHIFT
							&& (e.stateMask & SWT.ALT) == SWT.ALT && (e.keyCode == 'r')) {
						...
						if (RServe.isAliveDialog()) {
							...
							&& (e.stateMask & SWT.ALT) == SWT.ALT && (e.keyCode == 'o')) {
						...
						text.insert("-");
							...
							&& (e.stateMask & SWT.SHIFT) == SWT.SHIFT && (e.keyCode == 'n')) {
						// String a = t.substring(0, text.getCaretPosition());
						// String b = t.substring(text.getCaretPosition(), t.length());
						// text.setText(a + "<-" + b);
						text.insert(" %>% ");
		...
		source.addDragListener(new DragSourceListener() {
			public void dragStart(DragSourceEvent event) {
				if (listShell.getItemCount() > 0) {
					if (listShell.getSelectionIndex() >= 0) {

						if (listShell.getItem(listShell.getSelectionIndex()).length() != 0) {
							event.doit = true;
						} else {
							event.doit = false;
						}
	 ...
	 * com.eco.bio7.rbridge.RServe.printJob(tex)method has finished! If a package
	 * was detected the package list job is executed which calls (after finished)
	 * the job function RServeUtil.listRObjects(). If no package has been called the
	 * RServeUtil.listRObjects() is called to show new objects in the R-Shell view.
	 */
			...
			Job job = new Job("List Objects") {
				...
				protected IStatus run(IProgressMonitor monitor) {
					...
					dis.syncExec(new Runnable() {
						...
						public void run() {
							int tempIndex = listShell.getTopIndex();
							int[] selIndex = listShell.getSelectionIndices();
							listShell.setVisible(false);
							listShell.removeAll();
							listShell.setItems(listObjectsArray);
							listShell.setSelection(selIndex);
							listShell.setTopIndex(tempIndex);
