	...
	public class ContentProposalProvider implements IContentProposalProvider {
		...
		public IContentProposal[] getProposals(String contents, int position) {
			...
			if (contentLastCorr.contains("@") || contentLastCorr.contains("$")) {
				...
				colon = true;
				/*If we have only one colon (from to operator) we shorten the prefix to occurrence to get code completion for e.g., 1:length() or similar expressions!*/
				int index = contentLastCorr.indexOf(":");
				contentLastCorr = contentLastCorr.substring(index + 1, contentLastCorr.length());
				textLength = contentLastCorr.length();
			}
