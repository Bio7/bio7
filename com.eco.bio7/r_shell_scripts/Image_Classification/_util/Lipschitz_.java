...
/*Source from: https://imagej.nih.gov/ij/plugins/lipschitz/index.html*/
...
public class Lipschitz_ implements PlugInFilter {
	...
	static double m_Slope = 10; // slope
	...
	public void Lipschitz2D(ImageProcessor ip) {
		...
		for (int ii = 0; ii < m_channels; ii++) {
			for (int ij = 0; ij < ImageHeight * ImageWidth; ij++) {
				srcPixels[ii][ij] = (m_short ? sign * (tmpShortPixels[ii][ij] & 0xffff)
	...
	public void runLipschitz(ImageProcessor ip) {
		...
		imp2 = new ImagePlus(
				m_imp.getShortTitle() + " Filtered (Lipschitz) Slope:" + m_Slope + " " + ((m_Down) ? " -Down" : " ")
						+ " " + ((m_TopHat) ? " -TopHat" : " ") + ((breaked) ? " -INTERUPTED" : ""),
				m_stack_out);
		imp2.show();
		imp2.updateAndDraw();
		IJ.showProgress(1.0);
	...
	public static boolean isM_Down() {
		return m_Down;
	}

	public static void setDownHatFilter(boolean m_Down) {
		Lipschitz_.m_Down = m_Down;
	}

	public static boolean isM_TopHat() {
		return m_TopHat;
	}

	public static void setTopHatFilter(boolean m_TopHat) {
		Lipschitz_.m_TopHat = m_TopHat;
	}
	...
	public static void setSlopeFilter(double m_Slope) {
		Lipschitz_.m_Slope = m_Slope;
	}
