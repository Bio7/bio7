...
import com.eco.bio7.reditor.preferences.template.PythonContextType;
...
public class TemplateEditorUI  {
	...
	public TemplateStore getTemplateStore() {
		if (fStore == null) {
			fStore= new ContributionTemplateStore(getContextTypeRegistry(), PythonEditorPlugin.getDefault().getPreferenceStore(), CUSTOM_TEMPLATES_KEY);
			try {
				fStore.load();
			} catch (IOException e) {
				PythonEditorPlugin.getDefault().getLog().log(new Status(IStatus.ERROR, "org.eclipse.ui.examples.javaeditor", IStatus.OK, "", e)); //$NON-NLS-1$ //$NON-NLS-2$
			}
		}
		return fStore;
	}
	...
	public ContextTypeRegistry getContextTypeRegistry() {
		if (fRegistry == null) {
			...
			fRegistry.addContextType(PythonContextType.XML_CONTEXT_TYPE);
		}
		return fRegistry;
	}
	...
	public ImageRegistry getImageRegistry() {
		return PythonEditorPlugin.getDefault().getImageRegistry();
	}
	...
	public IPreferenceStore getPreferenceStore() {
		return PythonEditorPlugin.getDefault().getPreferenceStore();
	}

	public void savePluginPreferences() {
		PythonEditorPlugin.getDefault().savePluginPreferences();
	}
